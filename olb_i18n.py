...
MESSAGES: dict[str, dict[str, str]] = {
    "en": {
        ...
        "config_intro": "The first run writes your upstream settings. You can rerun `olb init` any time to update them. Example values only show the format; if a prompt shows your own address or parameters, that value is already saved.",
    ...
    "zh": {
        ...
        "config_intro": "首次启动会写入你的上游配置，之后可随时重新执行 `olb init` 修改。示例值仅用于演示格式；如果提示里出现你自己的地址或参数，那就是当前已保存值。",
